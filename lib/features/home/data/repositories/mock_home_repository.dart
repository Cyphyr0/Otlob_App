...
class MockHomeRepository implements HomeRepository {
  ...
  Future<List<Restaurant>> getRestaurants() async {
    ...
    if (_unsplashService != null && AppConfig.unsplashAccessKey.isNotEmpty) {
      final restaurantsWithImages = <Restaurant>[];
      for (final restaurant in _mockRestaurants) {
        final imageUrl = await _unsplashService.getFoodImageUrl(
          restaurant.cuisine,
        );
        final updatedRestaurant = restaurant.copyWith(imageUrl: imageUrl);
  ...
  Future<List<Restaurant>> searchRestaurantsPaginated(
    ...
    final allResults = await searchRestaurants(query);
    final startIndex = (page - 1) * limit;
    final endIndex = startIndex + limit;
